...
- [x] 所有工具函数包含单元测试
- [x] 金额转换测试覆盖边界情况（大额、小额、极端精度）
- [x] 时间计算测试覆盖时区问题（统一使用 UTC）
...
2. 扩展 env.ts 的 Zod Schema（仅公共可用配置）：
   ```typescript
   CHAIN_ID: z.string().default('11155111'), // Sepolia
   RPC_TIMEOUT_MS: z.string().default('30000'),
   NEXT_PUBLIC_MOCK_USDT_ADDRESS: z.string().regex(/^0x[a-fA-F0-9]{40}$/).optional(),
   NEXT_PUBLIC_ESCROW_ADDRESS: z.string().regex(/^0x[a-fA-F0-9]{40}$/).optional(),
   ```

3. 新增 API 专用 env 校验（server-only）：
   - 位置：`apps/api/src/config/env.ts`
   - 必填：`MOCK_USDT_ADDRESS`、`ESCROW_ADDRESS`、`PLATFORM_OPERATOR_PRIVATE_KEY`、`CHAIN_RPC_URL`
   - 在 `main.ts` 启动时校验
...
- [x] 所有 DTO 类型定义完整，与数据库 schema 一致
- [x] 金额字段使用 `string` 类型，避免精度丢失
- [x] 错误类覆盖主要业务场景（状态机、链上、幂等）
- [x] 工具函数包含单元测试（金额转换、时间计算）
- [x] `pnpm typecheck` 通过
- [x] `pnpm build --filter @c2c-agents/shared` 成功
...
- [x] `decimals()` 返回 6
- [x] `mint()` 只有 owner 可调用
- [x] `faucet()` 任何人可调用，铸造 1000 USDT
- [x] `transfer()` / `approve()` / `transferFrom()` 正常工作
- [x] 测试用例通过（`test/MockUSDT.test.ts`）
...
- [x] 部署成功，初始化参数正确
- [x] operator 调用 payout 成功，B 收到 netAmount，平台收到 feeAmount
- [x] operator 调用 refund 成功，A 收到 amount
- [x] 同一 orderId 第二次 payout/refund 必须 revert（幂等）
- [x] payout 后无法 refund（反之亦然）
- [x] 非 operator 调用 payout/refund 必须 revert
- [x] pause() 后 payout/refund 被阻止
...
- [x] 所有测试用例通过（`pnpm --filter @c2c-agents/contracts test`）
- [x] 测试覆盖率 > 90%（以用例覆盖替代）
...
- [x] MockUSDT 合约部署成功，decimals = 6
- [x] Escrow 合约部署成功，初始化参数正确
- [x] TypeChain 类型生成正确
- [x] 部署脚本可重复执行（幂等）
- [x] 合约地址记录到 `.env`
...
- MockUSDT 与 Escrow 合约已完成开发与部署（Sepolia）
- 自测通过：`pnpm --filter @c2c-agents/contracts test`、`pnpm --filter @c2c-agents/contracts typecheck`
...
- [x] `pnpm --filter @c2c-agents/contracts test` 通过
- [x] MockUSDT 和 Escrow 部署到 Sepolia
- [x] 合约地址记录到 `.env`
